var chainnetConfig = {
    mainnet: {
        name: "主网",
        contractAddress: "n21u1A4vAzAFQmRNcGVsiZhzpnFgnHk7gBd",
...
$(function ($) {
	...
	$("#loginbtn").on('click', function () {
		...
		var hash = sha256_digest(nickname+content);
		
		// send transaction
		var to = dappAddress;
		var value = "0";
		var callFunction = "createNew";
		var callArgs = "[\"" + nickname + "\",\"" + content + "\",\"" + hash + "\"]";
...
//	alert(JSON.stringify(resp));
	...
	if(result.length == 0){
		$("#userRow").append("您还没有发布任何寻友信息，请返回首页进行发布~</div>");
	}
	else{
		for(var i in result){
...
function publicSearchHandle(resp){
	...
	if(result.length == 0){
		$("#publicRow").append("当前还没有发布任何寻友信息，请返回首页进行发布~</div>");
	}
	else{
		for(var i in result){
